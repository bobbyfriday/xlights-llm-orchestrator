...
def test_beats_contrast_color_and_downbeat_emphasis():
    ...
    assert all(a.palette_colors == ["#FFF1A6"] for a in acc)        # beats = BRIGHTENED Gold (not the raw palette)
...
def test_chord_driven_accent_color():
    ...
    # no chords → single accent color
    one = place_beat_accents(sec, _rhythm(beats), ["04_BEAT_1", "04_BEAT_2"])
    assert {tuple(a.palette_colors) for a in one} == {("#FFF1A6",)}  # the lone brightened-Gold accent
