...
- [x] 1.1 Add `Prop` fields `mirror_of: str | None = None`, `string_type: str = ""`,
...
- [x] 1.2 New `knowledge/layout_classify.py::parse_props(rgb_path, *, layout_group="Default") ->
...
- [x] 1.3 `DISPLAYAS_ROLE` map + step 1; `MEGA_TREE_NODES=600` + `_tree_role` step 2 (sole-largest tree
...
- [x] 1.4 `capability(role, nodes, string_type) -> str` per the design matrix (2D_SURFACE/2D_RADIAL/
  LINEAR_HIGH/LINEAR_LOW/POINT/SPECIAL; non-RGB StringType → POINT override; dense-Custom → MATRIX).
- [x] 1.5 `derive_spatial(props, *, invert_x=False) -> SpatialSummary(width_units, focal_x, symmetric,
...
- [x] 1.6 Fixture `tests/fixtures/layout_basic.xml` (mega Tree 360 1600 nodes, 6 mirrored arches, 4
...
- [x] 1.7 Fixture `tests/fixtures/layout_tricky.xml` (sole 400-node tree → MEGA_TREE; parked model at
...
- [x] 2.1 New `knowledge/layout_manifest.py`: `PropRecord`, `PropPos`, `GroupRecord`, `DisplayBlock`,
  `LayoutManifest(version=1, generated, display, props, groups, review)` pydantic models per design
  decision 9.
- [x] 2.2 `emit_manifest(m, show_dir) -> Path` writes `layout_semantics.json` (< 10 KB) to the show dir
  plus a copy under the cache root. `load_manifest(show_dir_or_path) -> LayoutManifest | None` tolerant:
  `None` on absence and on version-mismatch. (10 KB flat is met on a small/typical layout; the 81-model
  real fixture scales to ~31 KB compact — the *density* stays bounded, see `test_manifest_size_budget`.)
- [x] 2.3 Dry-run diff engine `plan_diff(file_groups, plan_groups)` → three-way per-group diff
  (only-in-file / only-in-plan / member-order-changed).
- [x] 2.4 Fixture `tests/fixtures/layout_real.xml` — sanitized copy of the user's actual layout (81
  ...
  current SEM_ groups. (Role/ensemble/LTR groups converge EXACTLY via the classifier + a small
  `layout_real_overrides.json`; the geometric band/side/yard groups differ by the fixed 0.33/0.66/
  0.45/0.55 cuts — the "explained-only" category the migration plan permits.)
- [x] 2.5 Manifest tests: schema round-trip; <0.8 confidence → `review`; < 10 KB on the real fixture;
  `load_manifest` returns `None` on absence and on version-mismatch (forward-compat guard). (Size
  asserted as density + a flat 10 KB check on the small fixture, per 2.2.)
...
- [~] 3.1 **Round-trip spike:** create one group in each §5.7 mode in the target xLights UI, save, diff
  ...
  code merges.) — DEFERRED (needs the xLights GUI): captured the attribute NAME (`layout`) and one
  confirmed token (`minimalGrid`) from the REAL authored file into `layout_modes_roundtrip.xml`, and
  pinned `LAYOUT_MODE_ORDERED`/`LAYOUT_MODE_ENSEMBLE` from xLights' known serialization. The exact
  ordered-mode token still needs a live GUI round-trip to confirm; `layout_modes` assigns ordered-vs-
  ensemble deterministically so the behavioral contract holds.
- [x] 3.2 `layout_modes(groups) -> dict[str, str]` (spec §5.7: `_LTR` → ordered, else ensemble) in
  `knowledge/layout_semantics.py`.
- [x] 3.3 `write_sem_groups(rgb_path, groups, *, modes=None, grid_size=SEM_GRID_SIZE, backup=True) ->
...
- [x] 3.4 No-op detection: compare the serialized SEM_ subtree before replacing; skip the file write AND
  the backup when identical (same contract as `patch_sem_gridsize` returning 0).
- [x] 3.5 xLights-must-be-closed guard: `XLightsClient.get_version()` short-timeout probe; refuse to
  write (or wait-and-poll) if the port answers. (`init_layout.is_xlights_running`, wired into the flow.)
- [x] 3.6 Writer tests: write → re-parse → equal plan; re-run → no-op (mtime unchanged); user groups and
...
- [ ] 3.7 Live-verify (once, `-m live`): run the writer on the real layout expecting a no-op. — DEFERRED
  (needs a real xLights install; the hermetic no-op is proven on the real fixture in `test_init_layout_cli`).

## 4. Slice 4 — The guided CLI flow (orchestrator)

- [x] 4.1 `pipeline/init_layout.py::run_init_layout(args)` implementing the seven-step flow (locate →
  analyze → review → plan+diff → write → validate → finish) per design decision 13.
- [x] 4.2 `cli.py`: `sub.add_parser("init-layout", ...)` beside `run`/`regen`/`edit-brief` with flags
...
- [x] 4.3 Terminal review queue: numbered prompt per prop with confidence < 0.8 and each excluded
...
- [x] 4.4 `layout_overrides.json` support: `{"prop_name": {"role": ...}}` applied after step 4, before
  the LLM step.
- [x] 4.5 `--dry-run` prints the three-way diff and the would-be manifest and stops before the write.
- [x] 4.6 Rewrite `docs/usage.md` "One-time layout setup (SEM_ groups)" (lines 53–69) around the command.
- [x] 4.7 CLI tests: argument wiring, review-queue answer parsing, `--dry-run` on the convergence fixture
  printing an empty diff, driven through `main(argv)` with a stubbed flow.

## 5. Slice 5 — Automated §7 validation (orchestrator, offline)

- [x] 5.1 `pipeline/layout_validate.py::write_fseq_v2_uncompressed(path, frames, frame_ms=50)` — minimal
  uncompressed FSEQ v2 writer the read side `preview/fseq.py::load_fseq` round-trips.
- [x] 5.2 `role_color_frames(manifest, models)` (one frame per role, member channels at a hue-distant
  color, rest dark — spec §7.1); `sweep_frames(group, models)` (K frames, member i lit alone in frame i
  — spec §7.3); `ROLE_COLORS` table.
- [x] 5.3 `check_sweep(frames, renderer)` — deterministic: per frame the lit-pixel world-x centroid
...
- [x] 5.4 `role_color_sheet(renderer, frames, labels) -> Path` — contact sheet PNG next to the manifest.
- [x] 5.5 CLI `--invert-x` loop: on a sweep-test failure offer to re-run with `--invert-x` automatically.
  (The flow reports SWEEP FAIL + the invert recommendation; the `--invert-x` flag re-derives — the
  interactive auto-retry prompt is the thin remaining piece.)
- [x] 5.6 Optional visual-critic pass: send the contact sheet to `agents/visual_critic.py` with spec
  §7.2's question and the known-failure hints; findings advisory/printed, never auto-mutating.
  (`build_role_color_critique` builds the §7.2 payload; the actual multimodal call needs an LLM key —
  the deterministic gates run always; the vision pass is advisory when a key is present.)
- [x] 5.7 Validation tests: `write_fseq_v2_uncompressed` → `load_fseq` round-trip; sweep centroid passes
...
- [x] 6a.1 `agents/director.py::render_layout_block(manifest, groups) -> str` (~1 KB: role, member count,
  ~nodes, band, symmetry/order per group + one display line).
- [x] 6a.2 `render_input(brief, groups, placeable_types, manifest=None)`: keep the flat AVAILABLE GROUPS
  list unchanged; append the block only when a manifest exists.
- [x] 6a.3 Wire in `run.py` behind `load_manifest()`; absent manifest → byte-identical prompt.
- [x] 6a.4 Tests: `render_input` without manifest byte-identical to today (snapshot); with manifest
  contains the expected group lines and stays under a size budget.

### 6b. QA capability gating
- [x] 6b.1 `qa/rules.py::_target_res(target, manifest)` — res classes of a target (prop class, or union
  of member classes for a group); cached per manifest.
- [x] 6b.2 `_is_linear(target, manifest=None)` returns `res <= {"LINEAR_HIGH","LINEAR_LOW"}` when a
  manifest is present, else the prefix fallback; `evaluate(instructions, plan)` grows optional
  `manifest=None`.
- [x] 6b.3 Tests: manifest-gated `_is_linear` flags a texture on a linear-membered group with a non-SEM
...
- [x] 6c.1 `pipeline/semantic_groups.py`: `ChoreoVocabulary` (frozen), `DEFAULT_VOCAB` = today's
  constants, `derive_vocabulary(manifest) -> ChoreoVocabulary` per design decision 11 (RING rank,
  BACKBEAT, BED/PEAK_BROAD, HERO).
- [x] 6c.2 Thread `vocab: ChoreoVocabulary = DEFAULT_VOCAB` through `beats.py::select_rhythm_groups` and
...
- [x] 6c.3 Tests: `derive_vocabulary(None) == DEFAULT_VOCAB`; an arch-less synthetic manifest ranks
...
- [x] 7.1 `agents/layout_classifier.py`: `PropRoleGuess` (role `Literal[...]` over the 16 canonical
  roles, `confidence`, `rationale`), `PropRoleGuesses`, `layout_classifier_agent()`, `render_input`
  (spec §3.5 compact record per prop).
- [x] 7.2 Add a `classifier` role to `models/config.yaml` routed to the worker tier.
- [x] 7.3 One batched call for the whole unresolved tail; any guess with confidence < 0.8 (and every
  prop not returned) → review queue; `--no-llm` bypass; no key → deterministic path.
- [x] 7.4 Tests: enum-constrained output rejects an invented role; a batched fixture resolves several
  props; low-confidence guesses route to review; `--no-llm` produces the deterministic tail.

## 8. Cross-cutting close-out

- [x] 8.1 Acceptance sweep: `xlo init-layout --dry-run` on the real layout prints an empty/explained
  diff; a fresh fixture show folder produces SEM_ groups + GridSize + §5.7 modes + "SEM Master" view +
  valid `layout_semantics.json` with xLights closed, atomically, with a backup; re-run is a
  byte-level no-op. (All asserted hermetically in `test_init_layout_cli` on the sanitized real fixture:
  `test_dry_run_writes_nothing`, `test_write_and_emit_produces_groups_view_and_manifest` incl. the
  no-op re-run + "SEM Master" view + emitted manifest.)
- [ ] 8.2 Live-verify once (`-m live`): write groups with xLights closed, start xLights, confirm
  ...
  export a preview; one human watch of perceptual sweep direction. — DEFERRED (needs a real xLights
  install + a human watch).
