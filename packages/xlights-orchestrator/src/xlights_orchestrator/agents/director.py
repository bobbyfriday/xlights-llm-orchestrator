...
def render_input(brief: MusicBrief, groups: list[str], placeable_types: list[str]) -> str:
    ...
    return (
        ...
        + "\n\nProduce a rich CREATIVE BRIEF as a ShowPlan:\n"
          ...
          "- per section, PHRASING: set `phrasing` to 'legato' for calm, introspective, or"
          " melancholy sections — the cells get soft, evolving fades instead of crisp on/off edges,"
          " so they breathe rather than flash; set 'staccato' for energetic, punchy, rhythmic"
          " sections that want crisp hits. Leave it blank to default by the section's energy. You"
          " choose only the FEEL — code owns the actual fade/dissolve timing.\n"
