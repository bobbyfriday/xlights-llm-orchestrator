...
def render_input(brief: MusicBrief, groups: list[str], placeable_types: list[str]) -> str:
    ...
    return (
        ...
        + "\n\nProduce a rich CREATIVE BRIEF as a ShowPlan:\n"
          ...
          "- SECTION PACING: the song is already split into musical sections (below) — PREFER to keep"
          " that structure. One look held too long is BORING to a viewer: a section past ~35s reads"
          " as static. So keep sections short (≈10–35s) and let the show evolve. You MAY merge"
          " adjacent sections into one longer look when the music genuinely stays unified through"
          " them (a single sustained build or drone), but that's the exception — default to NOT"
          " merging, and a section over ~35s should be a deliberate, justified choice, not a"
